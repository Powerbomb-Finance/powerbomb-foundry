...
interface IPool {
    ...
    function add_liquidity(uint[2] memory amounts, uint _min_mint_amount, bool use_eth) external payable returns (uint);
    ...
    function remove_liquidity_one_coin(uint _token_amount, uint i, uint _min_amount, bool use_eth) external returns (uint);
