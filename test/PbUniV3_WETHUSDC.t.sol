...
contract PbUniV3Test_WETHUSDC is Test {
    PbUniV3 vault;
    ...
    IUniswapV3Pool uniswapV3Pool = IUniswapV3Pool(0xC31E54c7a869B9FcBEcc14363CF510d1c41fa443);
    ...
    function testDeposit() public {
        ...
        amountsOutMin[0] = quoter.quoteExactInputSingle(address(USDC), address(WETH), 500, 5000e6 / 2, 0);
        amountsOutMin[1] = 0;
        ...
        uint amountOutMin = quoter.quoteExactInputSingle(address(WETH), address(USDC), 500, WETH.balanceOf(address(vault)) / 2, 0);
        vault.reinvest(WETH, WETH.balanceOf(address(vault)), 0, 2000);
        // Second deposit with WETH reward
        amountsOutMin[0] = quoter.quoteExactInputSingle(address(USDC), address(WETH), 500, 4000e6 / 2, 0);
    ...
    function testWithdraw() public {
        ...
        // Withdraw (BTC)
        (uint amount0Min, uint amount1Min) = vault.getMinimumAmountsRemoveLiquidity(withdrawAmt, 500);
        uint[] memory amountsOutMin = new uint[](2);
        amountsOutMin[0] = quoter.quoteExactInputSingle(address(WETH), address(USDC), 500, amount0Min, 0);
        amountsOutMin[1] = 0;
        vault.withdraw(2500e6, address(WBTC), amount0Min, amount1Min, amountsOutMin);
        // Withdraw again
        vault.withdraw(2500e6, address(WBTC), amount0Min, amount1Min, amountsOutMin);
        // Withdraw (ETH)
        (amount0Min, amount1Min) = vault.getMinimumAmountsRemoveLiquidity(4000e6, 500);
        amountsOutMin[0] = quoter.quoteExactInputSingle(address(WETH), address(USDC), 500, amount0Min, 0);
    ...
    function _mockSwap() private {
        ...
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams(address(WETH), address(USDC), 500, address(this), block.timestamp, WETH.balanceOf(address(this)), 0, 0);
        ...
        swapRouter.exactInputSingle(params);
        deal(address(WETH), address(this), 0); // Reset WETH balanceOf address(this) to 0
    }
    ...
    function testHarvestToken1Only() public {
        ...
        USDC.approve(address(swapRouter), type(uint).max);
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams(address(USDC), address(WETH), 500, address(this), block.timestamp, USDC.balanceOf(address(this)), 0, 0);
        swapRouter.exactInputSingle(params);
        // Harvest
        vault.harvest();
        // Assertion check
    ...
    function testUpdateTicks() public {
        ...
        // Get parameters
        (int24 tickLower, int24 tickUpper) = vault.getTicks(3173104577100634406797925, 3507999100942997269317126);
        int24 tickSpacing = uniswapV3Pool.tickSpacing();
        ...
        uint amountOutMin = quoter.quoteExactInputSingle(address(USDC), address(WETH), 500, USDC.balanceOf(address(vault)), 0) * 95 / 1000;
