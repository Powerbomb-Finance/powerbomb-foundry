...
contract PbUniV3Test is Test {
    ...
    function setUp() public {
        vault = new PbUniV3();
        vault.initialize(500, address(this));
    }

    function test() public {
        // deal(address(WETH), address(vault), 1 ether);
        // deal(address(USDC), address(vault), 2000e6);
        deal(address(USDC), address(this), 4000e6);

        // uint160 sqrtPriceX96 = 3344745905857173187386410;
        ...
        // emit log_int(ticks[0]); // -203696
        // emit log_int(ticks[1]); // -199641

        // vault.deposit(-210000, -200000);
        USDC.approve(address(vault), type(uint).max);
        vault.deposit(USDC, 4000e6, 0, 2000, ticks[0], ticks[1]);

        // uint amountOut = quoter.quoteExactInputSingle(address(WETH), address(USDC), 500, WETH.balanceOf(address(vault)), 0);
        // emit log_uint(amountOut); // 346822338

        // vault.reinvest(WETH, WETH.balanceOf(address(vault)), 0, 2000);

        // emit log_uint(WETH.balanceOf(address(vault)));
        // emit log_uint(USDC.balanceOf(address(vault)));
        // uint amountOut = quoter.quoteExactInputSingle(address(WETH), address(USDC), 500, WETH.balanceOf(address(vault)), 0);
        // emit log_uint(amountOut); // 346822338

        deal(address(WETH), address(this), 1 ether);
        WETH.approve(address(vault), type(uint).max);
        vault.deposit(WETH, 1 ether, 0, 5000, 0, 0);
        ...
        deal(address(WETH), address(this), 1 ether);
        WETH.approve(address(swapRouter), type(uint).max);
        params = ISwapRouter.ExactInputSingleParams(address(WETH), address(USDC), 500, address(this), block.timestamp, 1 ether, 0, 0);
        swapRouter.exactInputSingle(params);
        USDC.approve(address(swapRouter), type(uint).max);
        params = ISwapRouter.ExactInputSingleParams(address(USDC), address(WETH), 500, address(this), block.timestamp, USDC.balanceOf(address(this)), 0, 0);
        swapRouter.exactInputSingle(params);
        ...
        (,,,,,,,uint128 liquidity ,,,,) = nonfungiblePositionManager.positions(vault.tokenId());
        // emit log_uint(liquidity); // 706010112582100
        vault.withdraw(USDC, liquidity, 0, 0, 0);


        // (uint160 sqrtPriceX96_,int24 tick,,,,,) = uniswapV3PoolState.slot0();
        // uint160 sqrtPriceX96 = TickMath.getSqrtRatioAtTick(tick);
        // emit log_uint(sqrtPriceX96); // 3344745905865437161733003
        // emit log_uint(sqrtPriceX96_); // 3344764130489276538792327
        // emit log_int(tick); // -201464
        // emit log_int(uniswapV3PoolState.tickSpacing()); // 10
        // emit log_address(uniswapV3PoolState.token0()); // WETH
        // emit log_address(uniswapV3PoolState.token1()); // USDC

        // uint sqrtPrice = uint(sqrtPriceX96) * uint(sqrtPriceX96);
        // uint numerator = sqrtPrice * 1e18;
        // uint denominator = 2**192 * 1e6;
        // uint price = numerator / denominator;
        // emit log_uint(price); // 1792

        // uint numerator_ = price * denominator;
        // uint sqrtPrice_ = numerator_ / 1e18;
        // uint sqrtPriceX96_ = 
        // emit log_uint();
    }
}
