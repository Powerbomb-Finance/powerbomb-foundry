...
import "../src/PbCvxLusd.sol";
import "../interface/IPool.sol";
import "../interface/IZap.sol";

contract PbCvxLusdTest is Test {
    ...
    IERC20Upgradeable lusd = IERC20Upgradeable(0x5f98805A4E8be255a32880FDeC7F6728C6568bA0);
    IERC20Upgradeable lqty = IERC20Upgradeable(0x6DEA81C8171D0bA574754EF6F8b412F2Ed88c54D);
    ...
    PbCvxLusd vaultWeth;
    PbCvxLusd vaultUsdc;
    IPool pool = IPool(0xEd279fDD11cA84bEef15AF5D39BB4d4bEE23F0cA);
    ...
    function setUp() public {
        ...
        PbCvxLusd vaultImpl = new PbCvxLusd();
        PbProxy proxy;
        uint pid = 33;
        ...
        vaultWbtc = PbCvxLusd(payable(address(proxy)));
        ...
        vaultWeth = PbCvxLusd(payable(address(proxy)));

        lpToken = vaultWbtc.lpToken();
    }

    // function test() public {
    //     deal(address(lusd), address(this), 10000 ether);
    //     lusd.approve(address(vaultWbtc), type(uint).max);
    //     vaultWbtc.deposit(lusd, 10000 ether, 0);
    ...
    //     // uint pendingLqty = vaultWbtc.getPoolExtraPendingReward();
    //     // console.log(pendingLqty);
    //     deal(address(crv), address(vaultWbtc), 1.1 ether);
    //     deal(address(cvx), address(vaultWbtc), 1.1 ether);
    //     deal(address(lusd), address(vaultWbtc), 1.1 ether);
    //     vaultWbtc.harvest();
    ...
    //     vaultWbtc.withdraw(lusd, lpTokenAmt, 0);
    ...
    function testDeposit() public {
        ...
        assertEq(lusd.balanceOf(address(vaultWbtc)), 0);
    ...
    function testWithdraw() public {
        ...
        // withdraw lusd from vaultWbtc
        amountOut = zap.calc_withdraw_one_coin(address(pool), vaultWbtc.getUserBalance(address(this)) / 3, int128(0));
        // console.log(amountOut);
        vaultWbtc.withdraw(lusd, vaultWbtc.getUserBalance(address(this)) / 3, amountOut * 99 / 100);
        ...
        // console.log(lusd.balanceOf(address(this)));
        assertGt(lusd.balanceOf(address(this)), 0);
    ...
    function testHarvest() public {
        ...
        (lqtyReward) = vaultWbtc.getPoolExtraPendingReward();
        // console.log(crvReward);
        // console.log(cvxReward);
        // console.log(lqtyReward);
        assertGt(crvReward, 0);
        assertGt(cvxReward, 0);
        assertEq(lqtyReward, 0); // no lqty reward currently
        (crvReward, cvxReward) = vaultWeth.getPoolPendingReward();
        (lqtyReward) = vaultWeth.getPoolExtraPendingReward();
        // console.log(crvReward);
        // console.log(cvxReward);
        // console.log(lqtyReward);
        assertGt(crvReward, 0);
        assertGt(cvxReward, 0); 
        assertEq(lqtyReward, 0);

        // assume crv, cvx & lqty > 1 ether
        deal(address(crv), address(vaultWbtc), 1.1 ether);
        deal(address(cvx), address(vaultWbtc), 1.1 ether);
        deal(address(lqty), address(vaultWbtc), 1.1 ether);
        deal(address(crv), address(vaultWeth), 1.1 ether);
        deal(address(cvx), address(vaultWeth), 1.1 ether);
        deal(address(lqty), address(vaultWeth), 1.1 ether);
