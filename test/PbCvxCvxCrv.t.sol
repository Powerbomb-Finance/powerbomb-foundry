...
import "../src/PbCvxCvxCrv.sol";
import "../interface/IPool.sol";
import "../interface/IZap.sol";

contract PbCvxCvxCrvTest is Test {
    ...
    IERC20Upgradeable cvxCrv = IERC20Upgradeable(0x62B9c7356A2Dc64a1969e19C23e4f579F9810Aa7);
    PbCvxCvxCrv vaultWbtc;
    PbCvxCvxCrv vaultWeth;
    PbCvxCvxCrv vaultUsdc;
    IPool pool = IPool(0x9D0464996170c6B9e75eED71c68B99dDEDf279e8);
    ...
    function setUp() public {
        ...
        PbCvxCvxCrv vaultImpl = new PbCvxCvxCrv();
        PbProxy proxy;
        uint pid = 41;
        ...
        vaultWbtc = PbCvxCvxCrv(payable(address(proxy)));
        ...
        vaultWeth = PbCvxCvxCrv(payable(address(proxy)));
        ...
        vaultUsdc = PbCvxCvxCrv(payable(address(proxy)));
    ...
    //     vaultWbtc.deposit(crv, 10000 ether, 0);
    //     deal(address(cvxCrv), address(this), 10000 ether);
    //     cvxCrv.approve(address(vaultWeth), type(uint).max);
    //     vaultWeth.deposit(cvxCrv, 10000 ether, 0);
    //     deal(address(lpToken), address(this), 10000 ether);
    //     lpToken.approve(address(vaultUsdc), type(uint).max);
    //     vaultUsdc.deposit(lpToken, 10000 ether, 0);
    ...
    //     vaultWeth.withdraw(cvxCrv, vaultWeth.getUserBalance(address(this)), 0);
    //     vaultUsdc.withdraw(lpToken, vaultUsdc.getUserBalance(address(this)), 0);
    //     // console.log(crv.balanceOf(address(this)));
    //     // console.log(cvxCrv.balanceOf(address(this)));
    ...
    function testDeposit() public {
        ...
        // deposit crv into vaultWbtc
        deal(address(crv), address(this), 10000 ether);
        crv.approve(address(vaultWbtc), type(uint).max);
        amounts = [10000 ether, uint(0)];
        amountOut = pool.calc_token_amount(amounts, true);
        // console.log(amountOut);
        vaultWbtc.deposit(crv, 10000 ether, amountOut * 99 / 100);
        // console.log(vaultWbtc.getAllPool());

        // deposit cvxCrv into vaultWeth
        deal(address(cvxCrv), address(this), 10000 ether);
        cvxCrv.safeApprove(address(vaultWeth), type(uint).max);
        amounts = [uint(0), 10000 ether];
        amountOut = pool.calc_token_amount(amounts, true);
        // console.log(amountOut);
        vaultWeth.deposit(cvxCrv, 10000 ether, amountOut * 99 / 100);
        ...
        assertEq(cvxCrv.balanceOf(address(vaultWbtc)), 0);
        ...
        assertEq(cvxCrv.balanceOf(address(vaultWeth)), 0);
        ...
        assertEq(cvxCrv.balanceOf(address(vaultUsdc)), 0);
    ...
    function testWithdraw() public {
        ...
        // withdraw crv from vaultWbtc
        amountOut = pool.calc_withdraw_one_coin(vaultWbtc.getUserBalance(address(this)), int128(0));
        // console.log(amountOut);
        vaultWbtc.withdraw(crv, vaultWbtc.getUserBalance(address(this)), amountOut * 99 / 100);

        // withdraw cvxCrv from vaultWeth
        amountOut = pool.calc_withdraw_one_coin(vaultWeth.getUserBalance(address(this)), int128(1));
        // console.log(amountOut);
        vaultWeth.withdraw(cvxCrv, vaultWeth.getUserBalance(address(this)), amountOut * 99 / 100);
        ...
        // console.log(cvxCrv.balanceOf(address(this)));
        assertGt(cvxCrv.balanceOf(address(this)), 0);
        assertEq(crv.balanceOf(address(vaultWeth)), 0);
        assertEq(cvxCrv.balanceOf(address(vaultWeth)), 0);
        assertEq(lpToken.balanceOf(address(vaultWeth)), 0);
