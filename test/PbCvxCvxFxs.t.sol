...
import "../src/PbCvxCvxFxs.sol";
import "../interface/IPool.sol";
import "../interface/IZap.sol";

contract PbCvxCvxFxsTest is Test {
    ...
    IERC20Upgradeable fxs = IERC20Upgradeable(0x3432B6A60D23Ca0dFCa7761B7ab56459D9C964D0);
    IERC20Upgradeable cvxFxs = IERC20Upgradeable(0xFEEf77d3f69374f66429C91d732A244f074bdf74);
    PbCvxCvxFxs vaultWbtc;
    PbCvxCvxFxs vaultWeth;
    PbCvxCvxFxs vaultUsdc;
    IPool pool = IPool(0xd658A338613198204DCa1143Ac3F01A722b5d94A);
    ...
    function setUp() public {
        ...
        PbCvxCvxFxs vaultImpl = new PbCvxCvxFxs();
        PbProxy proxy;
        uint pid = 72;
        ...
        vaultWbtc = PbCvxCvxFxs(payable(address(proxy)));
        ...
        vaultWeth = PbCvxCvxFxs(payable(address(proxy)));
        ...
        vaultUsdc = PbCvxCvxFxs(payable(address(proxy)));

        lpToken = vaultWbtc.lpToken();
    }

    // function test() public {
    //     hoax(0x66df2139c24446f5B43dB80a680fb94D0c1c5D8E);
    //     fxs.transfer(address(this), 1000 ether);
    //     fxs.approve(address(vaultWbtc), type(uint).max);
    //     vaultWbtc.deposit(fxs, 1000 ether, 0);
    //     deal(address(cvxFxs), address(this), 1000 ether);
    //     cvxFxs.approve(address(vaultWeth), type(uint).max);
    //     vaultWeth.deposit(cvxFxs, 1000 ether, 0);
    ...
    //     vaultWbtc.withdraw(fxs, vaultWbtc.getUserBalance(address(this)), 0);
    //     vaultWeth.withdraw(cvxFxs, vaultWeth.getUserBalance(address(this)), 0);
    //     vaultUsdc.withdraw(lpToken, vaultUsdc.getUserBalance(address(this)), 0);
    //     // console.log(fxs.balanceOf(address(this)));
    //     // console.log(cvxFxs.balanceOf(address(this)));
    ...
    function testDeposit() public {
        ...
        // deposit fxs into vaultWbtc
        hoax(0x66df2139c24446f5B43dB80a680fb94D0c1c5D8E);
        fxs.transfer(address(this), 1000 ether);
        fxs.approve(address(vaultWbtc), type(uint).max);
        amounts = [1000 ether, uint(0)];
        amountOut = pool.calc_token_amount(amounts);
        // console.log(amountOut);
        vaultWbtc.deposit(fxs, 1000 ether, amountOut * 99 / 100);
        // console.log(vaultWbtc.getAllPool());

        // deposit cvxFxs into vaultWeth
        deal(address(cvxFxs), address(this), 1000 ether);
        cvxFxs.safeApprove(address(vaultWeth), type(uint).max);
        amounts = [uint(0), 1000 ether];
        amountOut = pool.calc_token_amount(amounts);
        // console.log(amountOut);
        vaultWeth.deposit(cvxFxs, 1000 ether, amountOut * 99 / 100);
        ...
        assertEq(fxs.balanceOf(address(vaultWbtc)), 0);
        assertEq(cvxFxs.balanceOf(address(vaultWbtc)), 0);
        ...
        assertEq(cvxFxs.balanceOf(address(vaultWeth)), 0);
        ...
        assertEq(cvxFxs.balanceOf(address(vaultUsdc)), 0);
    ...
    function testWithdraw() public {
        ...
        // withdraw fxs from vaultWbtc
        amountOut = pool.calc_withdraw_one_coin(vaultWbtc.getUserBalance(address(this)), uint(0));
        // console.log(amountOut);
        vaultWbtc.withdraw(fxs, vaultWbtc.getUserBalance(address(this)), amountOut * 99 / 100);

        // withdraw cvxFxs from vaultWeth
        amountOut = pool.calc_withdraw_one_coin(vaultWeth.getUserBalance(address(this)), uint(1));
        // console.log(amountOut);
        vaultWeth.withdraw(cvxFxs, vaultWeth.getUserBalance(address(this)), amountOut * 99 / 100);
        ...
        // console.log(fxs.balanceOf(address(this)));
        assertGt(fxs.balanceOf(address(this)), 0);
        assertEq(fxs.balanceOf(address(vaultWbtc)), 0);
        assertEq(cvxFxs.balanceOf(address(vaultWbtc)), 0);
        assertEq(lpToken.balanceOf(address(vaultWbtc)), 0);
        ...
        // console.log(cvxFxs.balanceOf(address(this)));
        assertGt(cvxFxs.balanceOf(address(this)), 0);
        assertEq(crv.balanceOf(address(vaultWeth)), 0);
        assertEq(cvxFxs.balanceOf(address(vaultWeth)), 0);
        assertEq(lpToken.balanceOf(address(vaultWeth)), 0);
    ...
    function testHarvest() public {
        ...
        (extraCvxReward) = vaultWbtc.getPoolExtraPendingReward(0);
        (fxsReward) = vaultWbtc.getPoolExtraPendingReward(1);
        // console.log(crvReward);
        // console.log(cvxReward);
        // console.log(extraCvxReward);
        // console.log(fxsReward);
        assertGt(crvReward, 0);
        assertGt(cvxReward, 0);
        assertGt(extraCvxReward, 0);
        assertGt(fxsReward, 0);

        // vaultWeth
        (crvReward, cvxReward) = vaultWeth.getPoolPendingReward();
        (extraCvxReward) = vaultWeth.getPoolExtraPendingReward(0);
        (fxsReward) = vaultWeth.getPoolExtraPendingReward(1);
        // console.log(crvReward);
        // console.log(cvxReward);
        // console.log(extraCvxReward);
        // console.log(fxsReward);
        ...
        (extraCvxReward) = vaultUsdc.getPoolExtraPendingReward(0);
        (fxsReward) = vaultUsdc.getPoolExtraPendingReward(1);
        // console.log(crvReward);
        // console.log(cvxReward);
        // console.log(extraCvxReward);
        // console.log(fxsReward);
        ...
        // assume crv & cvx > 1 ether & fxs > 0.5 ether
        deal(address(crv), address(vaultWbtc), 1.1 ether);
        deal(address(cvx), address(vaultWbtc), 1.1 ether);
        hoax(0x66df2139c24446f5B43dB80a680fb94D0c1c5D8E);
        fxs.transfer(address(vaultWbtc), 0.6 ether);
    ...
    function testPauseContract() public {
        ...
        vaultWbtc.deposit(cvxFxs, 1 ether, 0);
        vm.expectRevert(bytes("Pausable: paused"));
        vaultWeth.deposit(cvxFxs, 1 ether, 0);
        vm.expectRevert(bytes("Pausable: paused"));
        vaultUsdc.deposit(cvxFxs, 1 ether, 0);
        ...
        vm.stopPrank();
        deal(address(cvxFxs), address(this), 3 ether);
        cvxFxs.approve(address(vaultWbtc), type(uint).max);
        cvxFxs.approve(address(vaultWeth), type(uint).max);
        cvxFxs.approve(address(vaultUsdc), type(uint).max);
        vaultWbtc.deposit(cvxFxs, 1 ether, 0);
        vaultWeth.deposit(cvxFxs, 1 ether, 0);
        vaultUsdc.deposit(cvxFxs, 1 ether, 0);
        ...
        vaultUsdc.pauseContract();
        vm.stopPrank();
        vaultWbtc.withdraw(cvxFxs, vaultWbtc.getUserBalance(address(this)), 0);
        vaultWeth.withdraw(cvxFxs, vaultWeth.getUserBalance(address(this)), 0);
        vaultUsdc.withdraw(cvxFxs, vaultUsdc.getUserBalance(address(this)), 0);
    }
