...
contract PbUniV3Test_USDTUSDC is Test {
    PbUniV3 vault;
    ...
    IUniswapV3Pool uniswapV3Pool = IUniswapV3Pool(0x13398E27a21Be1218b6900cbEDF677571df42A48);
    ...
    function testDeposit() public {
        ...
        uint160 sqrtPriceX96Lower = 70863822845718284118440686464;
        uint160 sqrtPriceX96Upper = 86790103597495589788630435724;
        ...
        amountsOutMin[0] = quoter.quoteExactInputSingle(address(USDC), address(USDT), 500, 5000e6 / 2, 0) * 99 / 100;
        amountsOutMin[1] = 0;
        vault.deposit(5000e6, amountsOutMin, 2000, tick0, tick1, address(WBTC));
        // Bot reinvest
        // console.log(USDT.balanceOf(address(vault))); // 419.106548
        // console.log(USDC.balanceOf(address(vault))); // 0
        uint amountOutMin = quoter.quoteExactInputSingle(address(USDT), address(USDC), 500, USDT.balanceOf(address(vault)) / 2, 0) * 99 / 100;
        vault.reinvest(USDT, USDT.balanceOf(address(vault)), amountOutMin, 2000);
        // Second deposit with WETH reward
        amountsOutMin[0] = quoter.quoteExactInputSingle(address(USDC), address(USDT), 500, 4000e6 / 2, 0) * 99 / 100;
        vault.deposit(4000e6, amountsOutMin, 2000, 0, 0, address(WETH));
        // Bot reinvest
        // console.log(USDT.balanceOf(address(vault))); // 370.726345
        // console.log(USDC.balanceOf(address(vault))); // 0
        amountOutMin = quoter.quoteExactInputSingle(address(USDT), address(USDC), 500, USDT.balanceOf(address(vault)) / 2, 0) * 99 / 100;
        vault.reinvest(USDT, USDT.balanceOf(address(vault)), amountOutMin, 2000);
        // Assertion check
    ...
    function testWithdraw() public {
        ...
        uint[] memory amountsOutMin = new uint[](2);
        amountsOutMin[0] = quoter.quoteExactInputSingle(address(USDT), address(USDC), 500, amount0Min, 0);
        amountsOutMin[1] = 0;
        ...
        // Assertion check
        // console.log(USDC.balanceOf(address(this))); // 8964.320875
    ...
    function _mockSwap() private {
        deal(address(USDT), address(this), 300_000e6);
        // Swap WETH to USDC
        USDT.approve(address(swapRouter), type(uint).max);
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams(address(USDT), address(USDC), 500, address(this), block.timestamp, USDT.balanceOf(address(this)), 0, 0);
        swapRouter.exactInputSingle(params);
        // Swap USDC to WETH
        USDC.approve(address(swapRouter), type(uint).max);
        params = ISwapRouter.ExactInputSingleParams(address(USDC), address(USDT), 500, address(this), block.timestamp, USDC.balanceOf(address(this)), 0, 0);
        swapRouter.exactInputSingle(params);
        deal(address(USDT), address(this), 0); // Reset USDT balanceOf address(this) to 0
    }
    ...
    function testHarvestToken0Only() public {
        ...
        deal(address(USDT), address(this), 100000e6);
        USDT.approve(address(swapRouter), type(uint).max);
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams(address(USDT), address(USDC), 500, address(this), block.timestamp, USDT.balanceOf(address(this)), 0, 0);
        swapRouter.exactInputSingle(params);
        // Harvest
    ...
    function testHarvestToken1Only() public {
        ...
        USDC.approve(address(swapRouter), type(uint).max);
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams(address(USDC), address(USDT), 500, address(this), block.timestamp, USDC.balanceOf(address(this)), 0, 0);
        swapRouter.exactInputSingle(params);
        // Harvest
        vault.harvest();
        // Assertion check
    ...
    function testUpdateTicks() public {
        ...
        // Get parameters
        (int24 tickLower, int24 tickUpper) = vault.getTicks(75162434512514379355924140470, 83095197869223157896060286990);
        int24 tickSpacing = uniswapV3Pool.tickSpacing();
        ...
        // console.log(USDT.balanceOf(address(vault))); // 0
        // console.log(USDC.balanceOf(address(vault))); // 454.783715
        uint amountOutMin = quoter.quoteExactInputSingle(address(USDC), address(USDT), 500, USDC.balanceOf(address(vault)) / 2, 0) * 99 / 100;
