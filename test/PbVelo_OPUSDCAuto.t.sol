...
contract PbVeloTest_OPUSDCAuto is Test {
    PbVeloAuto vault;
    ...
    function testDistributeEvenly() public {
        // address(1) deposit
        deal(address(token0), address(1), 1 ether);
        startHoax(address(1));
        token0.approve(address(vault), type(uint).max);
        vault.deposit(token0, 1 ether, 0);
        deal(address(VELO), address(vault), 1000 ether);
        // Harvest
        vault.harvest();
        // address(2) deposit
        deal(address(token0), address(2), 1 ether);
        ...
        vault.harvest();
        // address(3) deposit
        deal(address(token0), address(3), 1 ether);
