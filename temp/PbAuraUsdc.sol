...
contract PbAuraUsdc is PbAuraBase {

    IChainlink constant usdcUsdPriceOracle = IChainlink(0x8fFfFfd4AfB6115b954Bd326cbe7B4BA576818f6);
    IChainlink constant ethUsdPriceOracle = IChainlink(0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419);
    bytes32 constant poolId = 0x96646936b91d6b9d7d0c47c496afbf3d6ec7b6f8000200000000000000000019; // usdc/weth balancer
    ...
    function deposit(
        ...
        require(token == weth || token == usdc || token == lpToken, "Invalid token");
    ...
    function getPricePerFullShareInUSD() public view override returns (uint) {
        ...
        (, int latestPrice,,,) = usdcUsdPriceOracle.latestRoundData(); // return 8 decimals
        uint usdcBalInUsd = balances[0] * uint(latestPrice) / 1e8; // return 6 decimals
        ...
        return (usdcBalInUsd + ethBalInUsd) * 1e18 / lpToken.totalSupply();
    }
