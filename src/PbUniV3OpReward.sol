...
contract PbUniV3OpReward is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable public constant WBTC = IERC20Upgradeable(0x68f180fcCe6836688e9084f035309E29Bf0A2095);
    ...
    function _harvest(
        ...
        assets[0] = address(rewardInfo[address(rewardToken)].ibRewardToken);
        rewardsController.claimRewards(assets, type(uint).max, address(this), address(OP));
        uint OPAmt = OP.balanceOf(address(this));
        if (OPAmt > 0) {
            rewardTokenAmt = _swap(address(OP), address(rewardToken), OPAmt);
        }
