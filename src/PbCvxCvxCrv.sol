...
contract PbCvxCvxCrv is PbCvxBase {
    ...
    function deposit(
        ...
        require(token == crv || token == cvxCrv || token == lpToken, "Invalid token");
        ...
        if (token != lpToken) {
            ...
            if (token == crv) amounts[0] = amount;
            else amounts[1] = amount; // token == cvxCrv
    ...
    function getPricePerFullShareInUSD() public view override returns (uint) {
        ...
        // get total crv
        uint totalCrv = pool.balances(0);
        // get total pool in crv
        uint totalPoolInCrv = totalCvxCrvInCrv + totalCrv;
        // get pricePerFullShareInCrv
        uint pricePerFullShareInCrv = totalPoolInCrv * 1e18 / lpToken.totalSupply();
        // get crv price in usd
        (, int latestPrice,,,) = crvUsdPriceOracle.latestRoundData(); // return 8 decimals
        
        return pricePerFullShareInCrv * uint(latestPrice) / 1e20;
    }
