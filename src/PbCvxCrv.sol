...
contract PbCvxCrv is PbCvxBase {
    ...
    function initialize(uint _pid, IPool _pool, IERC20Upgradeable _rewardToken) external initializer {
        ...
        crv.approve(address(pool), type(uint).max);
    ...
    function deposit(
        ...
        require(token == weth || token == crv || token == lpToken, "Invalid token");
        ...
        if (token != lpToken) {
            ...
            lpTokenAmt = pool.add_liquidity{value: msg.value}(amounts, amountOutMin, true);
        } else {
            lpTokenAmt = amount;
        }
    ...
    function withdraw(
        ...
        if (token != lpToken) {
            ...
            tokenAmt = pool.remove_liquidity_one_coin(lpTokenAmt, i, amountOutMin, useEth);
        } else {
            tokenAmt = lpTokenAmt;
        }
    ...
    function harvest() public override {
        ...
        gauge.getReward(address(this), true); // true = including extra reward if any
    ...
    function getPricePerFullShareInUSD() public view override returns (uint) {
        int latestPrice;
        // get total eth in usd
        (, latestPrice,,,) = ethUsdPriceOracle.latestRoundData(); // return 8 decimals
        uint totalEthInUsd = pool.balances(0) * uint(latestPrice) / 1e20; // return 6 decimals
        // get total crv in usd
        (, latestPrice,,,) = crvUsdPriceOracle.latestRoundData(); // return 8 decimals
        uint totalCrvInUsd = pool.balances(1) * uint(latestPrice) / 1e20; // return 6 decimals
        // get total pool in usd
        uint totalPoolInUsd = totalEthInUsd + totalCrvInUsd;
        
        return totalPoolInUsd * 1e18 / lpToken.totalSupply();
    }
